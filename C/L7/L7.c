...
#include <windows.h>
#include <math.h>
#define MARKS 5		//кол-во оценок за семестр
...
void ReadFromFile(Student **root, FILE *fp)
{
	while (!feof(fp))
	{
		...
		for (numOfTerms = 0; numOfTerms < st->course * 2; numOfTerms++)
		{
			...
			st->termMarks[numOfTerms].averageMark /= MARKS;
...
int main()
{
	...
	do
	{
		...
		puts("3 - Добавить студента");
		puts("4 - Показать всех студентов");
		puts("5 - Показать студентов _номер_ группы");
		puts("6 - Показать студентов _номер_ курса");
		puts("7 - Показать отметки студента");
		puts("8 - Показать всю информацию о студентах");
		puts("9 - Удалить студента");
		puts("0 - Удалить всех студентов");
		puts("q - Выход");
